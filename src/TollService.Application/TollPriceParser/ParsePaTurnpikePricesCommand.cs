...
public record PaTurnpikeTollEntry(
    [property: JsonPropertyName("ExitInterchangeId")] int ExitInterchangeId,
    ...
    IHttpClientFactory _httpClientFactory) : IRequestHandler<ParsePaTurnpikePricesCommand, ParseTollPricesResult>
{
    ...
    public async Task<ParseTollPricesResult> Handle(ParsePaTurnpikePricesCommand request, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.TollType))
            queryParams.Add($"tollType={Uri.EscapeDataString(request.TollType)}");
        ...
        if (!string.IsNullOrWhiteSpace(request.EntryInterchangeKey))
            queryParams.Add($"entryInterchangeKey={Uri.EscapeDataString(request.EntryInterchangeKey)}");
        if (!string.IsNullOrWhiteSpace(request.EffectiveDateKey))
            queryParams.Add($"effectiveDateKey={Uri.EscapeDataString(request.EffectiveDateKey)}");
        ...
        Toll? fromToll = null;
        if (!string.IsNullOrWhiteSpace(request.EntryInterchangeKey) && 
            int.TryParse(request.EntryInterchangeKey, out var entryInterchangeKeyInt))
        {
            fromToll = await FindTollByNumberInBounds(entryInterchangeKeyInt, paBoundingBox, paCalculator.Id, ct);
            if (fromToll == null)
            {
        ...
        foreach (var priceEntry in response.Data.ResultObject)
        {
            ...
            if (priceEntry.L2Axle == 0 && priceEntry.H2Axle == 0)
            {
                continue;
            }

            // Находим exit toll по ExitInterchangeId (Number)
            var toToll = await FindTollByNumberInBounds(priceEntry.ExitInterchangeId, paBoundingBox, paCalculator.Id, ct);
            if (toToll == null)
            {
            ...
            // Используем L2Axle для IPass/Online (легковой автомобиль, EZ-Pass)
            // Используем H2Axle для Cash (если отличается, иначе используем L2Axle)
            var ipassPrice = priceEntry.L2Axle;
            var onlinePrice = priceEntry.L2Axle;
            var cashPrice = priceEntry.H2Axle > 0 ? priceEntry.H2Axle : priceEntry.L2Axle;
            ...
            if (existingPrice != null)
            {
                ...
                existingPrice.IPass = ipassPrice;
                existingPrice.Online = onlinePrice;
            }
            else
            {
    ...
    /// </summary>
    private async Task<Toll?> FindTollByNumberInBounds(int number, Polygon boundingBox, Guid stateCalculatorId, CancellationToken ct)
    {
