    ...
    IHttpClientFactory _httpClientFactory) : IRequestHandler<ParseExitPointsToPlazasCommand, ParseExitPointsToPlazasResult>
{
    public async Task<ParseExitPointsToPlazasResult> Handle(ParseExitPointsToPlazasCommand request, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var queryUrl =
                $"{request.QueryUrl}?f=json&where=1%3D1%20AND%201763601274966%3D1763601274966&returnGeometry=true&spatialRel=esriSpatialRelIntersects&outFields=LABEL%2CGUID%2Cidnum%2CExitPointsSort&outSR=3857";
            ...
            foreach (var exitPoint in exitPointsResponse.Features)
            {
                ...
                try
                {
                    ...
                    foreach (var result in identifyResult.Results)
                    {
                        ...
                        // Создаем точку плазы
                        _ = new Point(plazaLon, plazaLat) { SRID = 4326 };
                        ...
                        if (existingTolls.Count > 0)
                        {
                            ...
                            foreach (var existingToll in existingTolls)
                            {
                                ...
                                if (!string.IsNullOrWhiteSpace(result.Attributes.Plaza_Name) &&
                                    !string.Equals(existingToll.Name, result.Attributes.Plaza_Name,
                ...
                {
                    errors.Add(
                        $"Ошибка при обработке exit point {exitPoint.Attributes?.LABEL ?? "unknown"}: {ex.Message}");
                    processedExitPoints++;
                }
            }
        }
    ...
    private static string BuildIdentifyUrl(string baseUrl, double x, double y)
    {
        ...
        var url =
            $"{baseUrl}?f=json&tolerance=15&returnGeometry=true&returnFieldName=false&returnUnformattedValues=false&imageDisplay=1181,1271,96&geometry={geometryEncoded}&geometryType=esriGeometryPoint&sr=3857&mapExtent={mapExtent}&layers=visible:13";
