...
public record ParsePlazaNamesFromArcGisCommand(
    string ArcGisUrl =
        "https://gis.illinoisvirtualtollway.com/arcgis/rest/services/IVT/IllinoisVirtualTollway/MapServer/13/query")
...
{
    private const double ToleranceMeters = 10; // Допустимое расстояние для сопоставления (метров)
    ...
    public async Task<ParsePlazaNamesResult> Handle(ParsePlazaNamesFromArcGisCommand request, CancellationToken ct)
    {
        ...
        foreach (var batch in batches)
        {
            ...
            catch (Exception ex)
            {
                ...
                errors.Add(
                    $"Ошибка при обработке батча дорог ({batch.Length} шт.): {roadNames}{more}. Ошибка: {ex.Message}");
                processedRoads += batch.Length;
            }
        }
